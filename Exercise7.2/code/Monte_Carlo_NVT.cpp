...
void Averages(int iblk) //Print results for current block
{
   ...
   ofstream Gofr, Gave, Epot, Pres;
    ...
    for(int k=igofr;k<igofr+nbins;++k){		//g(t)
		double deltaVol = 4./3. * M_PI * (pow(bin_size*(k-igofr)+bin_size, 3) - pow(bin_size*(k-igofr), 3));
		stima_gofr = blk_av[k]/ ( blk_norm*rho*npart*deltaVol );
...
//g(t) average
    for(int k=igofr;k<igofr+nbins;++k){
	    Gave << iblk << "	" << glob_av[k]/(double)iblk << "	" << err_gofr << endl;
	 }
    cout << "----------------------------" << endl << endl;
